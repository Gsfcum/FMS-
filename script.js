...
// Handle File Upload for both Noting and Corresponding sides
function handleFileUpload(e, fileType) {
    e.preventDefault();
    console.log('handleFileUpload called with type:', fileType);
    
    const prefix = fileType === FILE_TYPE.NOTING ? 'noting' : 'corresponding';
    console.log('Using prefix:', prefix);
    
    // Get form elements with null checks
    const fileInput = document.getElementById(`${prefix}FileInput`);
    const fileNumber = document.getElementById(`${prefix}FileNumber`)?.value || '';
    const partNumber = document.getElementById(`${prefix}PartNumber`)?.value || '';
    const fileSubject = document.getElementById(`${prefix}FileSubject`)?.value || '';
    const creationDate = document.getElementById(`${prefix}CreationDate`)?.value || '';
    const closingDate = document.getElementById(`${prefix}ClosingDate`)?.value || '';
    const classification = document.getElementById(`${prefix}Classification`)?.value || '';
    const disposalYear = document.getElementById(`${prefix}DisposalYear`)?.value || '';
    
    // Additional fields for corresponding files
    let referenceNumber = '';
    let source = '';
    let notes = '';
    let department = '';
    ...
    if (fileType === FILE_TYPE.CORRESPONDING) {
        ...
        const deptSelect = document.getElementById('correspondingDepartment');
        if (deptSelect) {
            department = deptSelect.value;
            console.log('Corresponding department:', department);
        } else {
            console.error('Department select not found');
        }
    } else if (fileType === FILE_TYPE.NOTING) {
        notes = document.getElementById('notingNotes')?.value || '';
        const deptSelect = document.getElementById('notingDepartment');
        if (deptSelect) {
            department = deptSelect.value;
            console.log('Noting department:', department);
        } else {
            console.error('Department select not found');
        }
    }
    
    console.log('Form data:', { fileNumber, fileSubject, creationDate, classification, department });
    ...
    if (!fileNumber) {
        showToast('File number is required', 'error');
        return;
    }
    
    if (!fileSubject) {
        showToast('File subject is required', 'error');
        return;
    }
    
    if (!creationDate) {
        showToast('Creation date is required', 'error');
        return;
    }
...
function viewFile(id) {
    const file = files.find(f => f.id === id);
    if (file) {
        alert(`File Details:\nFile Number: ${file.fileNumber}\nSubject: ${file.fileSubject}\nUploaded by: ${file.uploader}`);
        // In a real application, this would open a detailed view or download the file
    }
}
